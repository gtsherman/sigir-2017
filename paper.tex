...
The results confirm that document expansion provides benefit over a baseline query likelihood run---no run performs significantly worse than the baseline, and all expansion runs in AP and all but one expansion run in Robust improve over the baseline query likelihood run. % Note that baselines correspond to $\sum_{j=1}^n \lambda_{{\mathcal{E}_D}_j} = 0.0$. 
